...
## 3. Gemini APIのレート制限とモデル切り替え
Gemini APIにはレート制限があります（特に無料枠の場合）。
`RATE_LIMIT_EXCEEDED` (429) エラーが発生した場合、システムは自動的に以下の順序でモデルを切り替えてリトライを行います。
1. gemini-2.5-flash-lite
2. gemini-2.5-flash
3. gemini-3-flash-preview
4. gemma-3-27b-it

すべてのモデルで失敗した場合、ユーザーには「AIサービスのレートリミットに到達しました」というメッセージが返されます。
なお、設定されているモデル名の一部（gemini-2.5やgemini-3など）は将来のリリースを想定したものであり、現時点で存在しない場合はAPIエラーとなり、次のモデルへスキップされるか、翻訳が失敗する可能性があります。
